...
* :class:`~apps.tenants.models.Client` and :class:`~apps.tenants.models.Domain`
...
CLIENT_CHANGELIST = "/admin/tenants/client/"
...
@pytest.fixture
def acme_admin(acme: Client) -> HttpClient:
...
@pytest.fixture
def platform_admin(public_tenant: Client) -> HttpClient:
...
class TestCatalogueIsPublicOnly:
    def test_platform_admin_can_reach_the_client_changelist(
        self,
    ...
    def test_tenant_admin_cannot_reach_the_client_changelist(
        self,
...
@pytest.fixture
def client_admin() -> ClientAdmin:
    return ClientAdmin(TenantModel, django_admin.site)


@pytest.mark.django_db
class TestClientAdminSafety:
    def test_schema_name_is_editable_only_before_the_schema_exists(
        ...
        assert "schema_name" in client_admin.get_readonly_fields(request, acme)
        assert "schema_name" not in client_admin.get_readonly_fields(request, None)
    ...
    def test_admin_delete_keeps_the_schema(
        ...
        client_admin.delete_model(request, acme)
    ...
    def test_bulk_admin_delete_also_keeps_the_schemas(
        ...
        client_admin.delete_queryset(request, queryset)
